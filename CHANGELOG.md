...
- `boot()` -- allocates ONE shared buffer sized to the largest listener count, so
  `getAllInto` can never `RangeError`, then boots the container if needed.
  Idempotent.
- `emit(eventName, payload)` -- synchronous dispatch, 0 bytes/emit (hard-gated at
  zero): one `getAllInto` fill + an index loop, no `has()` pre-gate, no private
  cache, no try/catch in the hot body.
...
### Proven
- `node --expose-gc test/torture.mjs`: T0 dispatch laws, T3 lifecycle (A2), T5
  fuzz (32 seeds x 2000 iters, A6), T6 the 0 B/emit gate over 1e6 emits (A1) plus
  the recorded async lane, T7 soak (2000 cycles, lite-leak size 0, peak <= 2x
  baseline, A5), T9 controls (ASCII + per-emit alloc break, A3).
